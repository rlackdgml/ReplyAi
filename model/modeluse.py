...
def mongo_select_all():
    for one in collection.find({}, {'_id': 0, 'movieNm': 1, 'content': 1, 'score': 1}): # 제목,내용,점수
        reply_list.append([one['movieNm'], one['content'], one['score']])  #dict에서 value 와 scroe 의
    return reply_list
...
# 예측할 데이터의 백터화를 진행활 메서도(임베딩)
def term_frequency(doc):
    return [doc.count(word) for word in selected_words]

# 모델로 예측하는 메서드 구현
def predict_pos_neg(review):
    token = tokensize(review)
